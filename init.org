...
** Evil Commentary
~gc~ operator for commenting via evil-commentary (maps ~gc~, ~gcc~ etc. in normal/visual state).
...
** Modeline
Moody provides a minimal, flat underline-style modeline.
Minions collapses all minor modes into a single ~…~ menu.
#+begin_src emacs-lisp
    (use-package moody
      :config
...
** Smooth Scrolling
Ultra-scroll provides smooth pixel-level scrolling.
#+begin_src emacs-lisp
...
** Nyan Mode
Nyan cat progress indicator in the modeline (GUI only).
#+begin_src emacs-lisp
...
** Olivetti
Centers buffer content with comfortable margins — great for writing and presentations.
#+begin_src emacs-lisp
...
**** Org Agenda
Clean, minimal agenda styling — no visual clutter, gruvbox-aligned colours, generous spacing.
#+begin_src emacs-lisp
...
**** Presentations
Fullscreen presentation mode using org-tree-slide. Each top-level heading becomes a slide.
Speaker notes: add a =:NOTES:= drawer inside any heading — hidden from the slide view, shown in the notes frame.
Navigate slides with =n= / =p= in normal mode.
...
:NOTES:
Only you see this — talking points, timing cues, etc.
:END:
#+end_example
#+begin_src emacs-lisp
...
*** Markdown
[[https://jblevins.org/projects/markdown-mode/][markdown-mode]] provides syntax highlighting and editing support for Markdown files.
We pair it with a =pandoc=-powered preview that renders directly in an =eww= buffer — no browser needed, similar to =glow= in the terminal.
...
** AI Review
Review git changes with gptel. Grabs the relevant diff and opens an AI review in a dedicated buffer.
#+begin_src emacs-lisp
...
#+RESULTS:
: [nil 27053 51718 836978 nil elpaca-process-queues nil nil 0 nil]
...
** In-File Search
Magit-style transient popup for in-file search and replace.
Toggle case sensitivity, regexp mode, and whole-word matching before launching:
- *Search* — live minibuffer results via =consult-line= (navigate with C-j/C-k, RET to jump)
- *Occur* — persistent results buffer; navigate with =n=/=p= and =RET= to jump
- *Replace* — =query-replace= or =query-replace-regexp= honouring the current options

Activate with =SPC s s=.
#+begin_src emacs-lisp
  (require 'transient)
...
** Ace Window
Visual window switching with numbered labels. Press the binding and type the window number to jump to it.
#+begin_src emacs-lisp
